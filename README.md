...
Telegram-бот для художественной прозы и explicit/NSFW-фанфиков через Anthropic Claude API напрямую — без фильтров claude.ai. С памятью фактов о персонажах, умным роутингом моделей, кешированием промпта и учётом расходов.
...
Telegram bot for literary prose and explicit/NSFW fanfiction via the Anthropic Claude API directly — no claude.ai filters. With character-fact memory, smart model routing, prompt caching and cost tracking.
